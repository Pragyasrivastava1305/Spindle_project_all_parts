...
%%% iterations (20) for 4 values of D = [0 0.01 0.1 1]. It then calls
...
plt_kym = 0;  %%%% 1 for all cell kymographs if 
plt_save =0;  %%%% 1 for saving the plot
...
for iD =1:size(Dvec,2)
    ...
    for iter=1:Tot_iter
        [iD iter]
        param= data(iter,:);
        err(iD,iter) = Error_all_cell_Jan18(l,D,param,dxvec(iD));        
    end
    
    if plt_err ==1
    plot(1:Tot_iter,err(iD,:),'o','linewidth',1.5); hold on ; drawnow
    end
end
axis square; 
grid on; box on;
...
rc = linspace(0,1,20); 
if plt_min ==1
%%%%%  minimum appears for D=0.01, iter=2;
...
if plt_save ==1
    if plt_rates ==1
    xlim([0.5 3.5])
    saveas(gcf,['D=',num2str(D),'_rates.fig'])
    elseif plt_all ==1
    saveas(gcf,['D=',num2str(D),'_all_params.fig'])  
    xlim([0.5 15.5])
    end
end
...
if plt_kym ==1 
   ...
   for icell=1:12
       ...
       tes = Error_Cell_Jan18_single(dir,icell,sp_vec(icell),l,Dvec(iD),par_vec,dxvec(iD),'flag') 
       if plt_save ==1
