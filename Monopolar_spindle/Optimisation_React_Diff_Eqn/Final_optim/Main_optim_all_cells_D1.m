...
%%%% dynamics). 
dxvec = [0.05 0.05 0.05 0.5]; 
%%%%%% when we are comparing results from differnt D's then it is important
%%%%%% to take dx according to the vector dxvec, which ensures that the
%%%%%% time accuracy is same across differnt D's. But for single D runs it
%%%%%% is fine to take a bit larger dx so as it get faster runs. 
%%%%% with both, variable dx for each D as well as same dx for each D, D=1
%%%%% gives minimum error on manual checks.
