...
mu0 = 0.02;  %%% mobility = characteristic velocity*5*len_sp/Lm/L^2;
...
tau = 1/koffin;     %%%%  tau = 1/koffnear as defined in the supplementary. 
...
dx = 0.01; 
%%%%%% full cell_length p1*(1-tanh((tnew-p2)/p3))+p4: 
%%%%%% performed 3 params fit, fixing b =  22.6250
...
phi_9 = deg2rad(70);                           %%%%%%%%% ANGLE AT T= TROUND 
iter =1;  jreg =1; 
delt(iter) = 0.01;
time(iter) = 0; 
...
x = 0:dx(iter):L(iter);  
N = size(x,2); 
c_unif = ones(1,N);      %%%%% in units of c0_near = kon/koffnear. 
cL_old = c_unif;
phi_sp(iter) = phi_9;
...
while time(iter) < T_total 
    ...
    %%%%%%%%%%%%%% CALCULATE INHIBITED REGION ON PERIPHERY %%%%%%%%%%%%%%%%
    ...
    %%%%%%% EVOLE REACTION DIFFUSION EQUATION IF RAN INHITION IS ON.
    %lD = sqrt(D./koff); 
    if ran_inhibition ==1   
    ...
    [res_torque(iter),tdens1,tdens2,MT_ang1,MT_ang2] = Torque_static_pinned_Aug18(cell_a(iter),cell_b(iter),th,s_in...
                                        ,dth,dx(iter),rn,rs,phi_sp(iter),len_sp,Lm,x,L(iter),cL_old);
    %%%%%%%%%%%%%%%%%%%%% update angle %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    phi_sp(iter+1) = round(phi_sp(iter) + mu0*res_torque(iter)*delt(iter),rn);
...
% c_save = interp2(X0,T0,c_array',XS,TS);

save(['Dyn_sim_Phi_in=',num2str(rad2deg(phi_sp(1)))...
    ,'_mu=',num2str(mu0),'_dx1=',num2str(dx(1)),'.mat'])
