...
phi_10 = deg2rad(70);                           %%%%%%%%% ANGLE AT T= TROUND 
iter =1;  jreg =1; dx = 0.01; 
delt(iter) = 0.01;
time(iter) = 0; 
...
while time(iter) < T_total 
    ...
    if ran_inhibition ==1   
        ...
        cL1=cL_old + rk4_RDM(cL_old,lD,kon,koff,dtc,dx(iter));
        cLhalf=cL_old + rk4_RDM(cL_old,lD,kon,koff,0.5*dtc,dx(iter));
        cL2=cLhalf + rk4_RDM(cLhalf,lD,kon,koff,0.5*dtc,dx(iter));
...
%save(['Steady_state_phi=70_c0=30_dx=0.05.mat'])
