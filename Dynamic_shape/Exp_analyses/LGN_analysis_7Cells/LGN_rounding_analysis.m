...
%%% AND DEFINITIONS. 
%%% Need to run this section just once. 
Load_xls_file; 
...
% for icell = 1:size(cell_loc,2)-1
  icell  = 5
    ...
    sp_ang_vec = []; 
    %%%  regular theta grid for kymograph and binary kymograph
    ang_kym = linspace(0,360,360); 
    lgn_kym = zeros(360,size(t_arr,1)); 
    lgn_kym_bin = zeros(360,size(t_arr,1)); 
    
    for ifr =1:size(fr_loc,2)-1
    %%% for checks : initial and final points of s and angle. 
    %%% s should be [0, L(t)] and angle should be [theta_i, theta_f]
    %%% where theta_i is the orientation of current long axis of cell with
    %%% respect to its long axis in frame 1. 
    ...
    lgn_mat(1:diff_vec(ifr),(ifr-1)*3+1) = s_coord(fr_loc(ifr)+1:fr_loc(ifr+1)); 
    lgn_mat(1:diff_vec(ifr),(ifr-1)*3+2) = ang_coord(fr_loc(ifr)+1:fr_loc(ifr+1)); 
    lgn_mat(1:diff_vec(ifr),(ifr-1)*3+3) = lgn_norm(fr_loc(ifr)+1:fr_loc(ifr+1)); 
    ang_new = linspace(0,360,diff_vec(ifr)); 
    
    %%%%  for kymograph  
    lgn_kym(:,ifr) = interp1(ang_new,lgn_mat(1:diff_vec(ifr),(ifr-1)*3+3),ang_kym);   
    if t_arr(ifr) >= 0
    ...
    if opt_disp ==1
        dsize = 60;
        hold off
        %%% plot against the s coordinate
%             plot(s_coord(fr_loc(ifr)+1:fr_loc(ifr+1)), ...
%                     lgn_norm(fr_loc(ifr)+1:fr_loc(ifr+1)),...
%                       '.-.','color',[1 0 0],'linewidth',2 ,'Markersize',15)
        %%% plot lgn against theta coordinate
        plot(ang_new,lgn_norm(fr_loc(ifr)+1:fr_loc(ifr+1)),...
                      '.-.','color',[1 0 0],'linewidth',2 ,'Markersize',15)
        hold on; 
    ...
    plot(t_arr(4:end),sp_ang_vec,'o-.'...
        ,'linewidth',1.5,'MarkerFaceColor',[0 0.75 0]...
        ,'MarkerEdgeColor',[0 0 0],'markersize',8); drawnow; hold on;
    set(gca,'Fontsize',14); 
    xlabel('Time (min)', 'FontSize',16)
    ylabel('Spindle angle w.r.t initial long axis', 'FontSize',16)
    ...
    if plt_kym ==1
     figure;
     subplot(1,2,1)
     imagesc(t_arr,ang_kym,lgn_kym)   
     hold on 
     plot(t_arr(t_arr>=0),sp_ang_vec,'ok','MarkerSize',6,'MarkerFaceColor','g')
     plot(t_arr(t_arr>=0),mod(sp_ang_vec+180,360),'ok','MarkerSize',6,'MarkerFaceColor','g')
     colorbar
     axis xy; axis square
     xlabel('Time','Fontsize',14); 
     set(gca,'FontSize',12);
     ylabel('Angle','FontSize',14); 
     %saveas(gcf,['Bipolar_cell',num2str(icell),'_kym.png'])
     title('Kymograph Raw','FontWeight','normal')

     lgn_mean_kym = sum(sum(lgn_kym))/360/size(t_arr,1);
     for ifr =1:size(t_arr,1)
        lgn_kym_bin(lgn_kym(:,ifr)>=lgn_mean_kym,ifr) = 1; 
     end

     subplot(1,2,2)
     colormap(summer)
     imagesc(t_arr,ang_kym,lgn_kym_bin)   
     hold on 
     plot(t_arr(t_arr>=0),sp_ang_vec,'ok','MarkerSize',6,'MarkerFaceColor','g')
     plot(t_arr(t_arr>=0),mod(sp_ang_vec+180,360),'ok','MarkerSize',6,'MarkerFaceColor','g')
     ...
     set(gca,'FontSize',12);
     ylabel('Angle','FontSize',14); 
     title('Kymograph Binary','FontWeight','normal')
     saveas(gcf,['Kym_cell',num2str(icell),'.png'])
    end 
