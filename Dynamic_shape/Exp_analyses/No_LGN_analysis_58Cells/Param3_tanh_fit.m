function [fit_res,fit_opt,h2] = Param3_tanh_fit(quantity,b,norm_opt,col_mat,plt_save)
%%% Given 'quantity', this function plots the quantity for each cell,
%%% calculates mean and std over the cells and fits the mean curve by tanh

%%%% Inputs : quantity = quantity to be plotted,
%%%% norm_opt = 1 for normalised plots, 0 for raw
%%%% col_mat = A matrix of size(Ncell,3) specifying the colors of individual
%%%% lines, plt_save = 1 if individual plots are to be saved
%%%% b is the final value of the quantity, that is supplied from when
%%%% calling the function. 
%%%% Also need to change b value in the fit function by hand every time
...
%   [Dynamic_shape.id(loc(icell)+1) Dynamic_shape.id(loc(icell+1)+1)]    
    ...
    figure(1)
    if norm_opt == 0       
    plot(time,quant_dyn,'.-.','color',[R_vec(icell) G_vec(icell) B_vec(icell)],'linewidth',1.2,'Markersize',16)
    hold on; drawnow
    Lmat(icell,1:size(quant_dyn,1)) = quant_dyn;
    
    elseif norm_opt ==1
    norm_y = (quant_dyn(1:end)-quant_dyn(end))/(quant_I(icell)-quant_F(icell)); 
    plot(time,norm_y,'.-.','color',[R_vec(icell) G_vec(icell) B_vec(icell)],'linewidth',1.2,'Markersize',8); drawnow
    hold on
...
%%%%% change the constant to 1 (for AR) and 22.625 (for cell length and width)
%%%%% in the first argument (fit form) of the fittype function. 
ft = fittype( 'a*(1-tanh((x-C)/tau))+22.625', 'independent', 'x', 'dependent', 'y' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Display = 'Off';
opts.StartPoint = [0.75 0.9 0.9];
