...
if ran_inhibition ==1
    %%%%  save xgrid 
    dlmwrite(['Ran_Noise=',num2str(noi_str),'Xgrid_Niter=',num2str(niter)...
                    ,'Nserial=',num2str(nserial),'.csv'],xgrid')
    %%%%  save concentration 
    dlmwrite(['Ran_Noise=',num2str(noi_str),'Conc_Niter=',num2str(niter),...
                    'Nserial=',num2str(nserial),'.csv'],cL_arr)
    %%%% save cell shape changes and spindle angle in time
    Time_mat = [tarray' cell_len'  cell_wid' phi_sp_reg'];
    dlmwrite(['Ran_Noise=',num2str(noi_str),'Shape_Niter=',num2str(niter)...
                    ,'Nserial=',num2str(nserial),'.csv'],Time_mat)
else
    %%%%  save xgrid 
    dlmwrite(['No_Ran_Noise=',num2str(noi_str),'Xgrid_Niter=',num2str(niter)...
                    ,'Nserial=',num2str(nserial),'.csv'],xgrid')
    
    %%%% save cell shape changes and spindle angle in time
    Time_mat = [tarray' cell_len' cell_wid' phi_sp_reg'];
    dlmwrite(['No_Ran_Noise=',num2str(noi_str),'Shape_Niter=',num2str(niter)...
                    ,'Nserial=',num2str(nserial),'.csv'],Time_mat)
    
end
end 
  
%dlmwrite('No_ran_angle_data.csv',[tarray' phi_array]);  
