...
%%%% of length. Set ran_inhibition = 0 or 1 to switch Ran inhibition off
%%%% and on. set noi_str  to control noise strength.
clear all; 
...
plt_movie =0;   %%% movie
ran_inhibition =0;  %%%%  switch Ran inhibition on/off by setting this parameter to 1/0. 

%%%%% Noise parameters 
noi_str = 0.5;        %%%  strength of noise
noi_iter = 200;      %%%  number of simulations on noise
nserial = 1; 
%noi_vec = rand(1,1e8)-0.5;    %%%% center the noise on 0. 
...
%%%% initialise array to store angle information of all realisations over
%%%% noise. 
phi_array = zeros(size(tarray,2),noi_iter);

for niter=1:noi_iter
    
    rng(nserial*niter);
    seed =  rng; 
    noi_vec = rand(1,1e8)-0.5; 

    phi_9 = 0;                           %%%%%%%%% ANGLE AT T= TROUND
    iter =1;  jreg =1; dx = 0.02; 
    delt(iter) = 0.01;
    time(iter) = 0; 

    %%% assign cell half length and width according to experimental fit
    ...
    %%%%  time dependent quantities on irregular time array 
    ...
    %%%%  time dependent quantities on regular time array 
    ...
    %%%%% &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    %%%%% EVOLUTION BEFORE TROUND : SHAPE CHANGES + LGN PATTERNING,BUT NO SPINDLE 
    %%%%% DYNAMICS %%%%%%%%%%
    %%%% for tests
    ...
    %plot(x,cL_old); drawnow; hold on  
        %%%%%%%%%%%%%% inhibition range on periphery calculated separately
        %%%%%%%%%%%%%% for t < Tround and t >= Tround
        if time(iter) < Tround  
        ...
        %%%% concentration changes only because of rounding, as calculated
        %%%% later, outside this if condition. 
        cL1 = cL_old; 
        dtc = dtreg;
        err_step(iter) = 1;  
        %%% It does not mean anything for constant time, but is required
        %%% for the if in adaptive time step condition. Set it to its optimal value 1. 
        end
    ...
    eta(iter) = noi_str*noi_vec(randperm(length(noi_vec),1));
    ...
    %%%%%%%%%%%%%%%%%%%%% update angle %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    phi_sp(iter+1) = round(phi_sp(iter) + (mu0*res_torque+eta(iter))*delt(iter),rn);
...
phi_array(:,niter) = phi_sp_reg;

%%%%  write concentration in space and time for each iteration 
if ran_inhibition ==1    
    file_ind = (nserial-1)*noi_iter + niter; 
    dlmwrite(['Conc_with_Ran_Noise=',num2str(noi_str),'_Niter=',num2str(file_ind),'.csv'],cL_arr)   
% else: uncomment to save concentration for no-ran case. 
% By default code does not save it as it changes trivially according to cell 
%%%% shape change.
%     
%     dlmwrite(['Conc_No_Ran_Noise=',num2str(noi_str),'_Niter=',num2str(niter),'.csv'],cL_arr)
end

end 
  
%%%%  save xgrid and cell shape data outside the loop, since it is
%%%%  same for all iterations and is independent of Ran/No_ran conditions. 
Shape_mat = [tarray' cell_len' cell_wid']; 
dlmwrite(['Shape_data_Noise=',num2str(noi_str),'.csv'],Shape_mat);
dlmwrite(['Xgrid_data_Noise=',num2str(noi_str),'.csv'],xgrid);

%%%% save all angle data for all iterations in one file 
if ran_inhibition ==1
dlmwrite(['Angle_all_iters_with_Ran_Noise=',num2str(noi_str),'.csv'], [tarray' phi_array]); 
else 
dlmwrite(['Angle_all_iters_No_Ran_Noise=',num2str(noi_str),'.csv'], [tarray' phi_array]);     
end
