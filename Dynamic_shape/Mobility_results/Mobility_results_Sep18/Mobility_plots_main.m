%%%% This code generates additional plots of angle at 30 mins vs angle at 9
%%%% mins. When plt_kym=1, this code plots kymograph for specified value of
%%%% mu0 and initial angle. For opt_movie =1, this code produces movie of
%%%% the simulation for specified mu0 and initial angle. 
...
plt_indiv =1;
%plt_save =1
dir = 'Mobility_data_Sep18'; 
%mu0_vec = [1 0.1 0.01 0.002 0.001 0.0001 1e-5]; 
mu0_vec = [0.1 0.02 0.01 0.001 1e-5];
...
Tround =9; 
T_total = 100; 
tau_vec = 1./mu0_vec/11.31;

%%%% load common parameters for all data
com_data = load(fullfile(dir,['Time_quants_mu0=',num2str(mu0_vec(1)),'Phi_in=',num2str(rad2deg(0)),'.csv']));
...
%jmu = 4
    ...
    legend(['\tau_{sp}=',num2str(tau_vec(1)),' min'],['\tau_{sp}=',num2str(tau_vec(2)),' min'],...
            ['\tau_{sp}=',num2str(tau_vec(3)),' min'],['\tau_{sp}=',num2str(tau_vec(4)),' min'],...
            ['\tau_{sp}=',num2str(tau_vec(5)),' min'])
    ylabel('Spindle angle at 30 mins', 'Fontsize',16); ylim([0,90]);
    drawnow
  
end

%% specify mobility and initial angle to plot kymograph/ movie.

if (plt_kym ==1 || opt_movie ==1) 
