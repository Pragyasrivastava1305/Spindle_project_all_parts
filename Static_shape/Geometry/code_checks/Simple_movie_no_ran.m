...
ang_data= load(['Npoint=',num2str(Npoint),'Asp=',num2str(Asp_vec(iAsp)),'_Lm=',num2str(Lm)...
         ,'Phi_dyn_b=',num2str(b),'.dat']); 
phi_sp_reg =  ang_data(iphi,1:size(t_reg,2));
phi_nuc_reg= mod(phi_sp_reg+pi/2,2*pi);

mov = VideoWriter(['No_Ran_b=',num2str(b),'_Asp=',num2str(Asp_vec(iAsp)),'_iphi=',num2str(iphi),'.avi']);
